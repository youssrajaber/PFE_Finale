        ...
        @auth
            @if (Auth::user()->role === 'ADMIN')
                <div class="form-inline" style="width: 3rem">
                    <a href="{{ route('detailsADM', [auth()->user()->name, auth()->user()->email]) }}"><img
                            class="img-fluid rounded-circle" src="{{ asset('images/' . auth()->user()->image) }}"
                            alt="img"></a>
                </div>
            @else
                <div class="form-inline" style="width: 3rem">
                    <a href="{{ route('details', [auth()->user()->name, auth()->user()->email]) }}"><img
                            class="img-fluid rounded-circle" src="{{ asset('images/' . auth()->user()->image) }}"
                            alt="img"></a>
                </div>
            @endif
